...
function Videos() {
  ...
  const fetchVideos = async () => {
    ...
    if (res.status === "success") {
      setVideos(res.data)
      setCurrentIndex(0) // always start from first video
    }
  }
  ...
  return (
    ...
    <div className="card border-0 shadow-sm mb-4">
      <div className="card-body p-4">
        <h2 className="card-title text-center mb-4 fw-bold text-primary">{videos[0]?.course_name}</h2>
        ...
        <div className="col-lg-4">
          <ul className="list-group">
            {videos.map((v, i) => (
              <li
                key={i}
                className={`list-group-item ${i === currentIndex ? "active" : ""}`}
